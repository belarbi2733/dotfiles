...
- Vim
  - vimrc  
- OpenVPN
  - script to autoconnect to oprnvpn : 
  - Instructions :
    - sudo mkdir /opt/vpn
    - sudo mv script.sh /opt/vpn/auto_connect.sh
    - modify the third line with your VPN private IP (EX : 10.0.0.6)
  - To lunch the script open it in tmux :
    - tsudo bash /opt/vpn/auto_connect.sh
...
#+BEGIN_SRC sh
cd
git clone git@git.phoenix-ai.com:mohammed/dotfiles_vpn.git
